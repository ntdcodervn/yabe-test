...
struct Layout2: View {
    ...
    var body: some View {
        HStack(spacing : 4) {
            AnimatedImage(url: URL(string: services[0].download_url)).resizable().frame(width: width-(width/3), height: width-(width/3)+5, alignment: /*@START_MENU_TOKEN@*/.center/*@END_MENU_TOKEN@*/)
            VStack(spacing : 4) {
                if(services.count >= 2) {
